package co.veribilimi.spark.ml.squared.distance.vector.GMM
...
object GMM {
  def main(args: Array[String]): Unit = {
    ...
    def ComputeClusteringModel(df:DataFrame, k:Int): PipelineModel = {
      ...
      val gmmObject = new GaussianMixture().
        setSeed(142).
        setK(k).
      ...
      val pipeline = new Pipeline().setStages(
        Array(vectorAssembler, standartScaler, gmmObject))
    ...
    for(k <- 2 to 7 by 1){
      ...
      println("k value:"+k, "Score: "+score)
        ...
        val gmmModel = pipelineModel.stages.last.asInstanceOf[GaussianMixtureModel]
        ...
        val clusterCenters = gmmModel.weights
